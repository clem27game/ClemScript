...
typedef enum {
    ...
    TOKEN_INPUT, TOKEN_INTO, TOKEN_DELAY, // New
    TOKEN_IS_EVEN, TOKEN_IS_ODD, TOKEN_ROUND, // New (functions/operators)
...
typedef enum {
    ...
    NODE_IDENTIFIER,
    NODE_BINARY_EXPR,
    NODE_UNARY_OP_EXPR, // For 'not', 'is_even', 'is_odd', 'round'
    NODE_CONSOLE_STMT,
    NODE_VAR_DECL,
    NODE_ASSIGN_STMT,
    NODE_IF_STMT,
...
// Unary Operation Expression (for 'not', 'is_even', 'is_odd', 'round')
typedef struct {
    struct ASTNode *operand;
    TokenType operator_type; // TOKEN_NOT, TOKEN_IS_EVEN, TOKEN_IS_ODD, TOKEN_ROUND
} UnaryOpExpr;
...
// Input Statement: Clem input Script -> <prompt> Clem into Script <variable>
typedef struct {
...
// Expression parsing functions (updated for logical ops and new functions)
ASTNode *parse_expression(); // Entry point for expressions
...
ASTNode *parse_clem_function_call_or_primary(); // New parsing level for Clem-prefixed functions
ASTNode *parse_primary(); // Base literals/identifiers/parenthesized expressions
ASTNode *parse_block();
...
void free_ast_node(ASTNode *node) {
    ...
    switch (node->type) {
        ...
        case NODE_BINARY_EXPR:
            ...
            break;
        case NODE_UNARY_OP_EXPR: // New
            free_ast_node(node->data.unary_op_expr.operand);
            break;
...
static TokenType lexer_check_keyword(const char *text) {
    ...
    if (strcmp(text, "input") == 0) return TOKEN_INPUT; // New
    if (strcmp(text, "into") == 0) return TOKEN_INTO;   // New
    if (strcmp(text, "delay") == 0) return TOKEN_DELAY; // New
    if (strcmp(text, "is_even") == 0) return TOKEN_IS_EVEN; // New
    if (strcmp(text, "is_odd") == 0) return TOKEN_IS_ODD;   // New
    if (strcmp(text, "round") == 0) return TOKEN_ROUND;     // New
    if (strcmp(text, "and") == 0) return TOKEN_AND;         // New
    if (strcmp(text, "or") == 0) return TOKEN_OR;           // New
    if (strcmp(text, "not") == 0) return TOKEN_NOT;         // New
...
bool check_clem_keyword_script_sequence(TokenType keyword_type) {
    int saved_lexer_pos = lexer_current_pos;
    int saved_lexer_line = lexer_current_line;
    
    // Simulate advancing to check the pattern
    Token temp_token_clem = lexer_get_token_struct();
    if (temp_token_clem.type != TOKEN_CLEM) {
        free_token_lexeme(&temp_token_clem);
        lexer_current_pos = saved_lexer_pos;
        lexer_current_line = saved_lexer_line;
        return false;
    }

    Token temp_token_keyword = lexer_get_token_struct();
    if (temp_token_keyword.type != keyword_type) {
        free_token_lexeme(&temp_token_clem);
        free_token_lexeme(&temp_token_keyword);
        lexer_current_pos = saved_lexer_pos;
        lexer_current_line = saved_lexer_line;
        return false;
    }

    Token temp_token_script = lexer_get_token_struct();
    if (temp_token_script.type != TOKEN_SCRIPT) {
        free_token_lexeme(&temp_token_clem);
        free_token_lexeme(&temp_token_keyword);
        free_token_lexeme(&temp_token_script);
        lexer_current_pos = saved_lexer_pos;
        lexer_current_line = saved_lexer_line;
        return false;
    }

    // Pattern matched. Clean up temporary tokens and restore lexer state.
    free_token_lexeme(&temp_token_clem);
    free_token_lexeme(&temp_token_keyword);
    free_token_lexeme(&temp_token_script);
    lexer_current_pos = saved_lexer_pos;
...
ASTNode *parse_clem_statement() {
    ...
    if (current_token.type == TOKEN_CONSOLE) {
        ...
        return quiz_node;
    } else if (current_token.type == TOKEN_INPUT) { // New: Clem input Script -> prompt Clem into Script var
...
// --- Expression parsing (Operator Precedence Parsing with new logical ops) ---
// expression  -> logical_or
// logical_or  -> logical_and ( "Clem or Script" logical_and )*
// logical_and -> logical_not ( "Clem and Script" logical_not )*
// logical_not -> ( "Clem not Script" ) logical_not | comparison
// comparison  -> term ( ( ">" | ">=" | "<" | "<=" | "==" | "!=" ) term )*
// term        -> factor ( ( "+" | "-" ) factor )*
// factor      -> clem_function_call_or_primary ( ( "*" | "/" ) clem_function_call_or_primary )*
// clem_function_call_or_primary -> "Clem (is_even|is_odd|round) Script" expression | primary
...
ASTNode *parse_logical_or() {
    ...
    while (current_token.type == TOKEN_CLEM && check_clem_keyword_script_sequence(TOKEN_OR)) {
        int node_line = current_token.line;
        consume(TOKEN_CLEM, "Clem");
        consume(TOKEN_OR, "or");
        consume(TOKEN_SCRIPT, "Script");

        ASTNode *node = create_node(NODE_BINARY_EXPR, node_line);
...
ASTNode *parse_logical_and() {
    ...
    while (current_token.type == TOKEN_CLEM && check_clem_keyword_script_sequence(TOKEN_AND)) {
        int node_line = current_token.line;
        consume(TOKEN_CLEM, "Clem");
...
ASTNode *parse_logical_not() {
    if (current_token.type == TOKEN_CLEM && check_clem_keyword_script_sequence(TOKEN_NOT)) {
        int node_line = current_token.line;
        consume(TOKEN_CLEM, "Clem");
        consume(TOKEN_NOT, "not");
        consume(TOKEN_SCRIPT, "Script");

        ASTNode *node = create_node(NODE_UNARY_OP_EXPR, node_line);
        node->data.unary_op_expr.operator_type = TOKEN_NOT;
        node->data.unary_op_expr.operand = parse_logical_not(); // `not` is right-associative
        return node;
    }
...
ASTNode *parse_factor() {
    ASTNode *expr = parse_clem_function_call_or_primary();
    while (current_token.type == TOKEN_ASTERISK || current_token.type == TOKEN_SLASH) {
        ASTNode *node = create_node(NODE_BINARY_EXPR, current_token.line);
        node->data.binary_expr.left = expr;
        node->data.binary_expr.operator = current_token.type;
        advance_token(); // Consume operator
        node->data.binary_expr.right = parse_clem_function_call_or_primary();
        expr = node;
    }
    return expr;
}

// New parsing level for "Clem (is_even|is_odd|round) Script" expressions
ASTNode *parse_clem_function_call_or_primary() {
    if (current_token.type == TOKEN_CLEM) {
        int node_line = current_token.line;
        // Check for 'is_even', 'is_odd', 'round' specifically.
        // `check_clem_keyword_script_sequence` peeks without consuming.
        if (check_clem_keyword_script_sequence(TOKEN_IS_EVEN)) {
            consume(TOKEN_CLEM, "Clem"); consume(TOKEN_IS_EVEN, "is_even"); consume(TOKEN_SCRIPT, "Script");
            ASTNode *node = create_node(NODE_UNARY_OP_EXPR, node_line);
            node->data.unary_op_expr.operator_type = TOKEN_IS_EVEN;
            node->data.unary_op_expr.operand = parse_expression(); // Allow full expression as argument
            return node;
        } else if (check_clem_keyword_script_sequence(TOKEN_IS_ODD)) {
            consume(TOKEN_CLEM, "Clem"); consume(TOKEN_IS_ODD, "is_odd"); consume(TOKEN_SCRIPT, "Script");
            ASTNode *node = create_node(NODE_UNARY_OP_EXPR, node_line);
            node->data.unary_op_expr.operator_type = TOKEN_IS_ODD;
            node->data.unary_op_expr.operand = parse_expression();
            return node;
        } else if (check_clem_keyword_script_sequence(TOKEN_ROUND)) {
            consume(TOKEN_CLEM, "Clem"); consume(TOKEN_ROUND, "round"); consume(TOKEN_SCRIPT, "Script");
            ASTNode *node = create_node(NODE_UNARY_OP_EXPR, node_line);
            node->data.unary_op_expr.operator_type = TOKEN_ROUND;
            node->data.unary_op_expr.operand = parse_expression();
            return node;
        }
        // If it's `Clem` but not one of these expression keywords, it's a syntax error in this context.
        // It's likely the start of a statement, which is not allowed within an expression.
        syntax_error("expected an expression, found 'Clem' not followed by a valid expression keyword (is_even, is_odd, round).");
    }
    // If not a Clem-prefixed function call, parse as a regular primary expression
...
Value evaluate(ASTNode *node) {
    ...
    switch (node->type) {
        ...
        case NODE_UNARY_OP_EXPR: { // New for 'not', 'is_even', 'is_odd', 'round'
            Value operand_val = evaluate(node->data.unary_op_expr.operand);
            result.type = VALUE_INT; // All these ops return integers (0 or 1)

            if (operand_val.type != VALUE_INT) {
                 fprintf(stderr, "Runtime Error [Line %d]: Unary operator (type %d) requires an integer operand.\n", node->line, node->data.unary_op_expr.operator_type);
                 exit(1);
            }

            switch (node->data.unary_op_expr.operator_type) {
                case TOKEN_NOT:
                ...
                case TOKEN_ROUND:
                    ...
                    // This function would be more meaningful if floating-point numbers were supported.
                    result.data.int_val = operand_val.data.int_val;
                    break;
                default:
